...
void init_config(py::module &m) {
    ...
    py::class_<GeneralOptions>(m, "GeneralOptions")
        .def_readwrite("device", &GeneralOptions::device)
    ...
    py::class_<StorageOptions>(m, "StorageOptions")
        ...
        .def_readwrite("edges_dtype", &StorageOptions::edges_dtype)
    ...
    py::class_<ReportingOptions>(m, "ReportingOptions")
        ...
        .def_readwrite("log_level", &ReportingOptions::log_level);
    ...
    m.def("parseConfig", [](string config_path) {
        const char *argv[] = {"marius_train", config_path.c_str()};
	    marius_options = parseConfig(2, (char **)argv);
